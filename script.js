...
async function fetchDiscordUser(token) {
    ...
    try {
        ...
        if (user.username) {
            ...
            document.getElementById('nav-username').innerText = user.global_name || user.username;
            
            // Nascondiamo il banner di verifica perché ora siamo identificati
            ...
            // Salviamo l'avatar nel database per farlo vedere nelle liste
            if (isCloudActive) {
                const safeNick = nick.replace(/\./g, '_');
                db.ref(`authorized_staff/${safeNick}`).update({ avatar: avatarUrl });
            }

            // SALVIAMO LA SESSIONE COMPLETA PER EVITARE RE-LOGIN
            currentUser.pfp = avatarUrl;
            localStorage.setItem('wash_rp_session', JSON.stringify(currentUser));
            sessionStorage.setItem('staff_auth', 'true'); // Rinforziamo lo stato auth
            
            // Se eravamo bloccati nel login, nascondiamolo ora che abbiamo l'utente
            const loginOverlay = document.getElementById('login-overlay');
            if (loginOverlay) loginOverlay.style.display = 'none';
            const dashboard = document.getElementById('dashboard');
            if (dashboard) dashboard.style.display = 'flex';
...
async function updateStaffList() {
    ...
    if (activeStaff.length === 0) {
        ...
        activeStaff.forEach(name => {
            const avatar = authorizedData[name.replace(/\./g, '_')]?.avatar || 'assets/logo.png';
            activeBody.innerHTML += `
                <tr>
...
async function submitSanc() {
    ...
    try {
        ...
        addGlobalLog(`${currentUser.name} ha applicato un ${type.toUpperCase()} a ${subject} **firmato**: ${currentUser.name}`, 'sanctions');
...
function renderStaffTable(staffArray, statusMap) {
    ...
    body.innerHTML = staffArray.map(s => {
        ...
        const statusClass = isOnline ? 'background: rgba(46,204,113,0.1); color: #2ecc71;' : 'opacity: 0.6;';
        ...
        return `
            <tr>
                <td style="display: flex; align-items: center; gap: 10px;">
                    <img src="${avatar}" style="width: 25px; height: 25px; border-radius: 50%; object-fit: cover;">
                    <span style="font-weight: 600;">${s.nick}</span>
                </td>
                <td>${s.date}</td>
                <td><span class="status-tag" style="background: rgba(123,44,191,0.1); color: var(--accent-purple); border: 1px solid var(--glass-border);">${s.rank || 'N/D'}</span></td>
