...
function checkDiscordCallback() {
    // 1. Controlliamo se abbiamo già un token salvato in questa sessione
    const savedToken = sessionStorage.getItem('discord_access_token');
    if (savedToken) {
        fetchDiscordUser(savedToken);
        return;
    }

    // 2. Altrimenti controlliamo l'URL (hash)
...
async function fetchDiscordUser(token) {
    ...
    try {
        ...
        if (user.username) {
            ...
            // AGGIORNIAMO L'UI DELLA DASHBOARD IMMEDIATAMENTE
            const navName = document.getElementById('nav-username');
            const navPfp = document.getElementById('nav-pfp');
            if (navName) navName.innerText = currentUser.displayName;
            if (navPfp) navPfp.src = avatarUrl;
            ...
            if (isCloudActive) {
                ...
                if (!snap.exists()) {
                    // REGISTRAZIONE AUTOMATICA (Primo Accesso)
                    await db.ref(`authorized_staff/${safeNick}`).set({
                        nick: nick,
                        discordId: user.id,
                        avatar: avatarUrl,
                        date: new Date().toLocaleDateString(),
                        rank: "Staff da Verificare", 
                    ...
                    await db.ref(`authorized_staff/${safeNick}`).update({ 
                        avatar: avatarUrl,
                        discordId: user.id
                    });
                    const existingData = snap.val();
                    currentUser.rank = existingData.rank || "Staff";
                }
            }
            ...
            // Mostriamo Dashboard (usando la funzione corretta ora)
            showDashboard();
            
            addGlobalLog(`Staff ${nick} si è identificato tramite Discord.`, 'access');
        } else {
            throw new Error("Impossibile recuperare username Discord");
        }
    } catch (e) {
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fab fa-discord"></i> RETRY / IDENTIFICATI';
        }
        showModal("Errore Discord", "Non è stato possibile verificare la tua identità Discord. Riprova o contatta l'amministratore.", null, false, true);
    }
}

// --- LOGICA SICUREZZA DASHBOARD (5 TENTATIVI) ---

function unlockPasswordArea(nick, displayName) {
    document.getElementById('login-user').value = nick;
    
    // FORZIAMO IL CAMBIO DI STATO NELLA SCHEDA
    const authState = document.getElementById('auth-state');
    const passState = document.getElementById('pass-state');
    
    // Sparisce il bottone Discord
    if (authState) authState.style.display = 'none';
    
    // Appare solo il campo Password
    if (passState) {
        passState.style.display = 'block';
        // Focus automatico così puoi scrivere subito
        setTimeout(() => {
    ...
    document.getElementById('login-title').innerText = `CIAO, ${displayName || nick}`;
    document.getElementById('login-subtitle').innerText = "Identità confermata! Inserisci la chiave staff.";
}
...
async function verifySecurityPassword() {
    ...
    try {
        ...
        if (pass === correctPass) {
            ...
            // MOSTRA IL POPUP DI VERIFICA (COME RICHIESTO)
            if (!currentUser || !currentUser.name || currentUser.name.startsWith('Staff')) {
                showModal(
                    "<span style='background: linear-gradient(to right, #7b2cbf, #ff006e); -webkit-background-clip: text; -webkit-text-fill-color: transparent;'><i class='fab fa-discord'></i> VERIFICA IDENTITÀ</span>", 
                    "Per poter firmare le sanzioni e sbloccare il tuo profilo, devi identificarti con Discord.", 
                    () => { loginWithDiscord(); }, 
                    false,
                    true // NASCONDE IL TASTO ANNULLA
                );
                // Cambiamo lo stile del bottone del modal per questo caso specifico
                setTimeout(() => {
                    const modalBtn = document.getElementById('modal-confirm-btn');
                    if (modalBtn) {
                        modalBtn.innerText = "IDENTIFICATI ORA";
                        modalBtn.className = "discord-btn-premium";
                        modalBtn.style.width = "100%";
                    }
                }, 100);
            }
            
            addGlobalLog(`Accesso effettuato con password staff.`, 'access');
        } else {
            // Errore: Contiamo i tentativi
            loginAttempts++;
        ...
        console.error(e);
        alert("Errore critico di sicurezza.");
    }
}
        ...
        pageItems.forEach((name, index) => {
            ...
            rankingBody.innerHTML += `
                <tr>
                    ...
                    <td style="display: flex; align-items: center; gap: 10px;">
                        <img src="${avatar}" style="width: 30px; height: 30px; border-radius: 50%; border: 2px solid var(--glass-border); object-fit: cover;">
                        <div style="display: flex; flex-direction: column;">
                            <strong style="font-size: 0.85rem;">${name}</strong>
                            <small style="opacity: 0.6; font-size: 0.7rem;">${rank}</small>
                        </div>
...
async function submitSanc() {
    ...
    try {
        ...
        addGlobalLog(`${currentUser.name} ha applicato un ${type.toUpperCase()} a ${subject} **firmato**: ${signature}`, 'sanctions');
        addNotification(subject, `Hai ricevuto un ${type.toUpperCase()}. Motivo: ${reason}. Firmato: ${signature}`);
        
        // Feedback Successo
        showModal("✅ Operazione Riuscita", `La sanzione (${type.toUpperCase()}) per <b>${subject}</b> è stata registrata correttamente.`, null);
...
async function searchSubject() {
    ...
    if (foundName) {
        ...
        logs.slice().reverse().forEach(log => {
            results.innerHTML += `
                <div class="log-entry" style="border-left-color: ${log.type === 'ban' ? 'var(--accent-red)' : 'var(--accent-purple)'}; display: flex; align-items: center; gap: 15px;">
                    <img src="${log.subjectAvatar || 'assets/logo.png'}" style="width: 45px; height: 45px; border-radius: 50%; border: 2px solid var(--accent-purple); object-fit: cover;">
                    <div style="flex: 1;">
                        <strong>${log.type.toUpperCase()}</strong> - ${new Date(log.date).toLocaleString()}<br>
                        <span>Motivo: ${log.reason}</span><br>
                        <small>Firmato: ${log.signature}</small>
                    </div>
                </div>`;
        });
...
async function processInactivity(id, status, reason = "") {
    ...
    addGlobalLog(`${currentUser.name} ha gestito la richiesta di ${userName} come ${status === 'approved' ? 'ACCETTATA' : 'RIFIUTATA'} **firmato**: ${currentUser.name}`, 'access');
    renderInactivityRequests();
...
async function submitStrike() {
    ...
    try {
        ...
        addGlobalLog(`${currentUser.name} ha assegnato uno STRIKE (${num}) a ${target} **firmato**: ${currentUser.name}`, 'sanctions');
...
function confirmRemoveInactivity(id, user) {
    showModal("Rimuovi Inattività", "Sei sicuro di voler eliminare questa richiesta?", async () => {
        ...
        addGlobalLog(`${currentUser.name} ha rimosso una richiesta inattività di ${user} **firmato**: ${currentUser.name}`, 'access');
        checkStaffStatus();
    });
}
...
function confirmRemoveStrike(id, user) {
    showModal("Rimuovi Strike", "Sei sicuro di voler togliere questo strike?", async () => {
        ...
        addGlobalLog(`${currentUser.name} ha rimosso uno strike a ${user} **firmato**: ${currentUser.name}`, 'sanctions');
        checkStaffStatus();
    });
}
...
async function registerStaff() {
    ...
    if (!date || !nick || !rank) { showModal("Errore", "Devi compilare tutti i campi (incluso il grado) per registrare lo staff!", null); return; }
    ...
    try {
        if (isCloudActive) {
            ...
            await db.ref(`staff/${nick}`).update({ hours: 0, status: "Offline", rank: rank });
        }
        addGlobalLog(`${currentUser.name} ha registrato lo staffer ${nick} con grado ${rank} **firmato**: ${currentUser.name}`, 'access');
        showModal("Staff Registrato", `Lo staffer ${nick} è stato registrato come ${rank} con successo!`, null);
...
async function removeStaff() {
    ...
    showModal("Conferma Rimozione", `Sei sicuro di voler eliminare @${nick} dallo staff? Non potrà più accedere.`, async () => {
        try {
            ...
            addGlobalLog(`${currentUser.name} ha rimosso lo staffer ${nick} dal sistema **firmato**: ${currentUser.name}`, 'access');
...
async function modifyStaffNick() {
    ...
    const newNick = document.getElementById('mod-staff-new').value.trim() || oldNick;
    const newRank = document.getElementById('mod-staff-rank').value;

    if (!reason || !oldNick) { showModal("Errore", "Compila almeno il motivo e il vecchio nick!", null); return; }
    if (newNick && !newNick.startsWith('@')) { showModal("Errore", "Il nuovo nick deve iniziare con @!", null); return; }
    ...
    try {
        if (isCloudActive) {
            ...
            if (snap.exists()) {
                const data = snap.val();
                let logMsg = `${currentUser.name} ha aggiornato ${oldNick}: `;
                ...
                if (newNick !== oldNick) {
                    ...
                    logMsg += `nuovo nick ${newNick}; `;
                }

                if (newRank) {
                    await db.ref(`authorized_staff/${safeNew}`).update({ rank: newRank });
                    await db.ref(`staff/${newNick}`).update({ rank: newRank });
                    logMsg += `nuovo grado ${newRank}; `;
                }
                
                logMsg += `**firmato**: ${currentUser.name}`;
                
                addGlobalLog(logMsg, 'access');
                showModal("Successo", "Dati staff aggiornati con successo!", null);
                ['mod-staff-reason', 'mod-staff-old', 'mod-staff-new', 'mod-staff-rank'].forEach(id => {
                    const el = document.getElementById(id);
                    if (el) el.value = '';
                });
            } else {
                showModal("Errore", "Lo staffer non è stato trovato nel sistema.", null);
            }
        }
    } catch (e) {
        console.error(e);
...
async function updateStaffListPage() {
    ...
    try {
        ...
        const staffArray = Object.values(authorized).sort((a, b) => a.nick.localeCompare(b.nick));
...
function renderStaffTable(staffArray, statusMap) {
    ...
    body.innerHTML = staffArray.map(s => {
        const status = statusMap[s.nick]?.status || "Offline";
        const isOnline = status === "In Servizio";
        const statusClass = isOnline ? 'background: rgba(46,204,113,0.1); color: #2ecc71; border: 1px solid #2ecc71;' : 'background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.4); border: 1px solid rgba(255,255,255,0.1);';
        const avatar = s.avatar || 'assets/logo.png';
        
        return `
            <tr>
                <td style="display: flex; align-items: center; gap: 12px;">
                    <img src="${avatar}" style="width: 35px; height: 35px; border-radius: 50%; border: 2px solid var(--accent-purple); object-fit: cover; box-shadow: 0 2px 8px rgba(0,0,0,0.3);">
                    <div style="display: flex; flex-direction: column;">
                        <span style="font-weight: 600; font-size: 0.9rem;">${s.nick}</span>
                        <small style="opacity: 0.5; font-size: 0.7rem;">ID: ${s.discordId || '---'}</small>
                    </div>
                </td>
                <td style="font-size: 0.85rem; opacity: 0.8;">${s.date}</td>
                <td><span class="status-tag" style="background: rgba(123,44,191,0.15); color: var(--accent-purple); border: 1px solid var(--accent-purple); font-weight: 600;">${s.rank || 'N/D'}</span></td>
                <td><span class="status-tag" style="${statusClass}">${status.toUpperCase()}</span></td>
            </tr>
        `;
    }).join('');
}
