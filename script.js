...
async function fetchDiscordUser(token) {
    ...
    try {
        ...
        if (user.username) {
            ...
            // AGGIORNIAMO L'UI DELLA DASHBOARD
            document.getElementById('nav-username').innerText = currentUser.displayName;
            document.getElementById('nav-pfp').src = avatarUrl;
            ...
            if (isCloudActive) {
                ...
                if (!snap.exists()) {
                    ...
                    await db.ref(`authorized_staff/${safeNick}`).set({
                        ...
                        rank: "Staff da Verificare", // Grado predefinito da cambiare poi in Admin
                        addedBy: "Auto-Registration",
                        timestamp: Date.now()
                    });
                    // Inizializziamo anche il nodo ore se non esiste
                    ...
                    // AGGIORNIAMO SOLO AVATAR E INFO DISCORD
                    await db.ref(`authorized_staff/${safeNick}`).update({ 
                        avatar: avatarUrl,
                        discordId: user.id
                    });
                    
                    // Prendiamo il grado dal database (non da Discord)
                    const existingData = snap.val();
                    currentUser.rank = existingData.rank || "Staff";
                }
            }

            // Nascondiamo il banner di verifica
            const banner = document.getElementById('internal-discord-banner');
...
function showDashboard() {
    const nick = document.getElementById('login-user').value;
    currentUser = { 
        name: nick || document.getElementById('nav-username').innerText, 
        pfp: discordTempAvatar || "assets/logo.png" 
    };
