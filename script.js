    ...
    try {
        ...
        if (user.username) {
            ...
            if (user.avatar) {
                ...
                document.getElementById('login-avatar').style.border = "3px solid var(--accent-purple)";
                document.getElementById('login-avatar').style.boxShadow = "0 0 15px rgba(123, 44, 191, 0.3)";
            }
...
async function modifyStaffNick() {
    ...
    try {
        if (isCloudActive) {
            ...
            if (snap.exists()) {
                const data = snap.val();
                let logMsg = `${currentUser.name} ha aggiornato ${oldNick} **firmato**: ${currentUser.name}: `;
