...
async function fetchDiscordUser(token) {
    ...
    try {
        ...
        if (user.username) {
            ...
            // CAMBIO STATO AUTOMATICO (PAGINA 2)
            unlockPasswordArea(nick, user.global_name || user.username);
            
            // Salviamo temporaneamente l'avatar di Discord
            if (user.avatar) {
                ...
                if (avatarImg) {
                    avatarImg.src = discordTempAvatar;
                    avatarImg.style.border = "3px solid var(--accent-purple)";
                }
            }
        } else {
...
async function verifySecurityPassword() {
    ...
    try {
        ...
        if (pass === correctPass) {
            // Successo: Entriamo nella dashboard
            showDashboard();
            addGlobalLog(`${currentUser.name} ha sbloccato la dashboard con successo. **firmato**: ${currentUser.name}`, 'access');
        } else {
