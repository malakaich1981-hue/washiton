const CONFIG = {
    MAIN_PASSWORD: "Washinton_staff2026",
    SERVICE_GOAL_MS: 4 * 60 * 60 * 1000, // 4 ore
    ROTATION_DAYS: 30,
    WEBHOOK_URL: "https://discord.com/api/webhooks/1502305049345527980/FpR2e6eZ1ydGV_lVtfJRbN3VQmUtygL3UPjS6TC4oAg-wxu9L5UMaPOVXZ7mM0O5b3PK",
    DISCORD_CLIENT_ID: "1504568108856246343",
    REDIRECT_URI: "https://washiton.onrender.com"
};

// --- FIREBASE CONFIGURATION ---
const firebaseConfig = {
    apiKey: "AIzaSyAcDuC3SZHMTS7asSBCNcX8eogYSVhjJ08",
    authDomain: "washington-dashboard.firebaseapp.com",
    databaseURL: "https://washington-dashboard-default-rtdb.europe-west1.firebasedatabase.app",
    projectId: "washington-dashboard",
    storageBucket: "washington-dashboard.firebasestorage.app",
    messagingSenderId: "937326759375",
    appId: "1:937326759375:web:aeca082bf4dc0a168e96ba",
    measurementId: "G-D1LTBG0DR5"
};

// --- STATE MANAGEMENT ---
let currentUser = null;
let discordTempAvatar = null;
let serviceInterval = null;
let totalActiveMs = 0;
let modalCallback = null;
let db = null;
let isCloudActive = false;
let currentRankingPage = 0;
const RANKING_PAGE_SIZE = 15;
let robloxTimeout = null;
const DEFAULT_ADMIN = "@70mich";
let loginAttempts = 0;
const MAX_ATTEMPTS = 5;
let selectedRobloxAvatar = null;

// --- INITIALIZATION ---
window.onload = async () => {
    initFirebase();
    
    // --- FIX LABIRINTO LOGIN ---
    // Se la password è già stata inserita in questa sessione (es. dopo il redirect di Discord)
    // saltiamo la schermata di login e mostriamo la dashboard
    if (sessionStorage.getItem('staff_auth') === 'true') {
        const loginOverlay = document.getElementById('login-overlay');
        const dashboard = document.getElementById('dashboard');
        if (loginOverlay) loginOverlay.style.display = 'none';
        if (dashboard) dashboard.style.display = 'flex';
    }

    checkSession();
    await syncData(); // Sync cloud data first
    updateStaffList();
    renderActiveBans();
    renderInactivityRequests();
    updateNotifUI();
    autoCheckExpiredInactivity();
    renderActiveInactivityList();
    checkDiscordCallback();

    // Ripristina l'ultima sezione aperta
    const lastSection = sessionStorage.getItem('current_section');
    if (lastSection) showSection(lastSection);
};

// --- LOGICA LOGIN ---
async function login() {
    const user = document.getElementById('login-username').value.trim();
    const pass = document.getElementById('login-password').value;
    const errorEl = document.getElementById('login-error');
    const btn = document.querySelector('.login-btn');

    if (!user || !pass) {
        showError("Inserisci nome utente e password.");
        return;
    }

    if (!user.startsWith('@')) {
        showError("Il nome utente deve iniziare con @.");
        return;
    }

    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ACCESSO IN CORSO...';
    }

    try {
        let correctPass = "Washinton_staff2026";
        if (isCloudActive) {
            const snap = await db.ref("config/passwords/staff").once("value");
            if (snap.exists()) correctPass = snap.val();
        }

        // Verifica se l'utente è registrato
        let staffData = null;
        if (isCloudActive) {
            const safeNick = user.replace(/\./g, '_');
            const authSnap = await db.ref(`authorized_staff/${safeNick}`).once("value");
            if (authSnap.exists()) {
                staffData = authSnap.val();
            } else if (user.toLowerCase() === DEFAULT_ADMIN.toLowerCase()) {
                // Autocreate admin if it's the first time
                staffData = {
                    nick: DEFAULT_ADMIN,
                    rank: "Founder",
                    avatar: "assets/logo.png",
                    date: new Date().toLocaleDateString()
                };
                await db.ref(`authorized_staff/${safeNick}`).set(staffData);
            }
        }

        if (!staffData) {
            showError("Nome non registrato. Contatta un Amministratore per procedere con il rank.");
            return;
        }

        if (pass === correctPass) {
            currentUser = {
                name: staffData.nick,
                displayName: staffData.nick,
                pfp: staffData.avatar || "assets/logo.png",
                rank: staffData.rank
            };

            localStorage.setItem('wash_rp_session', JSON.stringify(currentUser));
            sessionStorage.setItem('staff_auth', 'true');
            
            showDashboard();
            addGlobalLog(`Staff ${user} ha effettuato l'accesso.`, 'access');
        } else {
            showError("Password errata!");
        }
    } catch (e) {
        console.error("Errore login:", e);
        showError("Errore di connessione al database.");
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.innerText = "ACCEDI";
        }
    }
}

function showError(msg) {
    const errorEl = document.getElementById('login-error');
    if (errorEl) {
        errorEl.innerText = msg;
        errorEl.style.display = 'block';
        setTimeout(() => { errorEl.style.display = 'none'; }, 5000);
    }
}

// Rimossi login con discord e password secondaria legacy

function initFirebase() {
    if (firebaseConfig.apiKey !== "YOUR_API_KEY") {
        firebase.initializeApp(firebaseConfig);
        db = firebase.database();
        isCloudActive = true;
        console.log("🔥 Washington RP Realtime Database Online");
        
        // Controllo rotazione password
        checkPasswordRotation();

        // Listener per i LOG
        db.ref("logs").limitToLast(50).on("value", snapshot => {
            let logs = [];
            snapshot.forEach(child => { logs.unshift(child.val()); });
            renderGlobalLogs(logs);
        });

        // Listener per lo STAFF
        db.ref("staff").on("value", () => updateStaffList());
    } else {
        console.log("📂 Modalità Locale Attiva (Nessuna Config Firebase)");
    }
}


async function checkPasswordRotation() {
    if (!isCloudActive) return;
    const snap = await db.ref("config/passwords").once("value");
    const now = Date.now();
    
    if (!snap.exists()) {
        // Inizializzazione prima volta
        const initial = {
            staff: "Washinton_staff2026",
            admin: "WSHT_rp2026",
            lastRotation: now
        };
        await db.ref("config/passwords").set(initial);
        return;
    }

    const data = snap.val();
    const daysPassed = (now - data.lastRotation) / (1000 * 60 * 60 * 24);

    if (daysPassed >= CONFIG.ROTATION_DAYS) {
        const newStaff = "WSH_" + Math.random().toString(36).substring(2, 8).toUpperCase();
        const newAdmin = "ADM_" + Math.random().toString(36).substring(2, 8).toUpperCase();
        
        await db.ref("config/passwords").update({
            staff: newStaff,
            admin: newAdmin,
            lastRotation: now
        });

        sendRotationWebhook(data.staff, newStaff, data.admin, newAdmin);
    }
}

async function sendRotationWebhook(oldS, newS, oldA, newA) {
    if (CONFIG.WEBHOOK_URL === "YOUR_DISCORD_WEBHOOK_HERE") return;

    const embed = {
        username: "SISTEMA DI SICUREZZA WASHINGTON RP",
        avatar_url: "assets/logo.png", // NOTA: Per i Webhook serve un URL pubblico (es. Imgur o il sito hostato)
        embeds: [{
            title: "📢 ANNUNCIO: ROTAZIONE MENSILE CREDENZIALI",
            description: "Il protocollo di sicurezza ha generato le nuove chiavi di accesso obbligatorie. Le credenziali precedenti sono state revocate con effetto immediato.",
            color: 15158332, // Rosso/Oro serio
            fields: [
                { name: "📜 VECCHIA PASSWORD (STAFF)", value: `\`${oldS}\``, inline: true },
                { name: "📜 VECCHIA PASSWORD (ADMIN)", value: `\`${oldA}\``, inline: true },
                { name: "\u200B", value: "\u200B", inline: false }, // Spacer
                { name: "🔑 NUOVA PASSWORD (STAFF)", value: `**${newS}**`, inline: true },
                { name: "🔑 NUOVA PASSWORD (ADMIN)", value: `**${newA}**`, inline: true },
                { name: "\u200B", value: "\u200B", inline: false }, // Spacer
                { name: "⏳ SCADENZA", value: new Date(Date.now() + 30*24*60*60*1000).toLocaleDateString(), inline: true },
                { name: "🕒 ORARIO", value: new Date().toLocaleTimeString(), inline: true },
                { name: "⚠️ ATTENZIONE", value: "Nessuno deve pubblicare o condividere queste password esternamente. La violazione di questo protocollo comporterà sanzioni disciplinari gravi.", inline: false }
            ],
            footer: { text: "Washington RP Dashboard • Sistema Automatizzato di Sicurezza" },
            timestamp: new Date().toISOString()
        }]
    };

    fetch(CONFIG.WEBHOOK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(embed)
    });
}

async function syncData() {
    // Non necessario per RTDB dato che usiamo i listener .on()
}

// --- CLOUD HELPERS ---
async function cloudSave(path, data) {
    if (!isCloudActive) return;
    await db.ref(path).update(data);
}

async function cloudGet(path) {
    if (!isCloudActive) return null;
    const snapshot = await db.ref(path).once("value");
    return snapshot.exists() ? snapshot.val() : null;
}

// --- SESSION & AUTH ---
function checkSession() {
    const session = localStorage.getItem('wash_rp_session');
    if (session) {
        currentUser = JSON.parse(session);
        document.getElementById('nav-username').innerText = currentUser.displayName || currentUser.name;
        if (currentUser.pfp) document.getElementById('nav-pfp').src = currentUser.pfp;
        
        showDashboard();
        loadUserServiceTime();
    }
}

async function loadUserServiceTime() {
    if (!currentUser) return;
    if (isCloudActive) {
        const data = await cloudGet(`staff/${currentUser.name}`);
        totalActiveMs = data ? (data.hours || 0) : 0;
    } else {
        const staffData = JSON.parse(localStorage.getItem('wash_rp_staff_hours') || '{}');
        totalActiveMs = staffData[currentUser.name] || 0;
    }
    updateTimerUI(totalActiveMs);
}

// --- NAVIGATION ---
function toggleSidebar() {
    const sidebar = document.getElementById('sidebar');
    const overlay = document.getElementById('overlay');
    if (!sidebar || !overlay) return;
    sidebar.classList.toggle('active');
    overlay.style.display = sidebar.classList.contains('active') ? 'block' : 'none';
}

function showSection(sectionId) {
    document.querySelectorAll('.section').forEach(s => s.classList.remove('active'));
    document.querySelectorAll('.sidebar-item').forEach(i => i.classList.remove('active'));
    
    const targetSection = document.getElementById(sectionId);
    if (targetSection) targetSection.classList.add('active');
    
    const items = document.querySelectorAll('.sidebar-item');
    items.forEach(item => { 
        if (item.getAttribute('onclick') && item.getAttribute('onclick').includes(`'${sectionId}'`)) {
            item.classList.add('active');
        }
    });

    if (window.innerWidth <= 768) toggleSidebar();

    if (sectionId === 'staff-list') updateStaffListPage();
    if (sectionId === 'inattivita-attive') renderActiveInactivityList();

    // Persistenza Sezione
    sessionStorage.setItem('current_section', sectionId);

    // Auto-unlock admin area
    if (sectionId === 'admin' && sessionStorage.getItem('admin_auth') === 'true') {
        document.getElementById('admin-lock').style.display = 'none';
        document.getElementById('admin-content').style.display = 'block';
        renderGlobalLogs();
        renderInactivityRequests();
    }

    // Auto-focus admin password if entering admin section
    if (sectionId === 'admin' && document.getElementById('admin-lock').style.display !== 'none') {
        setTimeout(() => document.getElementById('admin-pass-input').focus(), 100);
    }
}

// --- CUSTOM MODAL ---
function showModal(title, message, onConfirm, showInput = false, hideCancel = false) {
    document.getElementById('modal-title').innerHTML = title;
    document.getElementById('modal-message').innerHTML = message;
    
    const confirmBtn = document.getElementById('modal-confirm-btn');
    const cancelBtn = confirmBtn.nextElementSibling; // Il tasto Annulla

    if (hideCancel && cancelBtn) cancelBtn.style.display = 'none';
    else if (cancelBtn) cancelBtn.style.display = 'inline-block';

    confirmBtn.onclick = () => {
        if (onConfirm) onConfirm();
        closeModal();
    };
    
    document.getElementById('custom-modal').style.display = 'flex';
}

function closeModal() {
    document.getElementById('custom-modal').style.display = 'none';
    modalCallback = null;
    // Reset buttons for next call
    const cancelBtn = document.querySelector('.modal-btns .btn-pause');
    if (cancelBtn) cancelBtn.style.display = "block";
}

// --- NOTIFICATIONS ---
function toggleNotifPanel() {
    const panel = document.getElementById('notif-panel');
    panel.style.display = panel.style.display === 'block' ? 'none' : 'block';
    if (panel.style.display === 'block') clearNotifBadge();
}

async function addNotification(userName, msg) {
    const notif = { msg, time: new Date().toISOString(), read: false };
    if (isCloudActive) {
        await db.ref(`notifications/${userName}`).push(notif);
    } else {
        let notifs = JSON.parse(localStorage.getItem('wash_rp_notifs') || '{}');
        if (!notifs[userName]) notifs[userName] = [];
        notifs[userName].unshift(notif);
        localStorage.setItem('wash_rp_notifs', JSON.stringify(notifs));
    }
    updateNotifUI();
}

async function updateNotifUI() {
    if (!currentUser) return;
    let userNotifs = [];
    if (isCloudActive) {
        const snapshot = await db.ref(`notifications/${currentUser.name}`).limitToLast(20).once("value");
        if (snapshot.exists()) {
            snapshot.forEach(child => { userNotifs.unshift(child.val()); });
        }
    } else {
        const notifs = JSON.parse(localStorage.getItem('wash_rp_notifs') || '{}');
        userNotifs = notifs[currentUser.name] || [];
    }
    
    const list = document.getElementById('notif-items-list');
    const badge = document.getElementById('notif-count');
    const unread = userNotifs.filter(n => !n.read).length;
    
    badge.innerText = unread;
    badge.style.display = unread > 0 ? 'block' : 'none';

    if (userNotifs.length === 0) {
        list.innerHTML = '<p style="font-size: 0.75rem; opacity: 0.5; text-align: center;">Nessuna nuova notifica</p>';
    } else {
        list.innerHTML = userNotifs.map(n => `
            <div class="notif-item" style="background: rgba(123,44,191,0.05); border-radius: 8px; margin-bottom: 8px; border-left: 3px solid var(--accent-purple);">
                <div style="font-weight:600;">⚠️ Avviso Sistema</div>
                <div style="margin: 4px 0;">${n.msg}</div>
                <small style="opacity:0.5;">${new Date(n.time).toLocaleString()}</small>
            </div>
        `).join('');
    }
}

async function clearNotifBadge() {
    if (!currentUser) return;
    if (isCloudActive) {
        // Semplicemente per comodità in RTDB le segniamo lette tutte insieme cancellando il badge locale o aggiornando il cloud
        // In questo caso per semplicità svuotiamo o aggiorniamo
    } else {
        let notifs = JSON.parse(localStorage.getItem('wash_rp_notifs') || '{}');
        if (notifs[currentUser.name]) {
            notifs[currentUser.name].forEach(n => n.read = true);
            localStorage.setItem('wash_rp_notifs', JSON.stringify(notifs));
        }
    }
    updateNotifUI();
}

// --- ADMIN MANAGEMENT FUNCTIONS ---
async function adminSearchSubject() {
    const query = document.getElementById('admin-search-input').value.trim().toLowerCase();
    const view = document.getElementById('admin-management-view');
    const title = document.getElementById('admin-target-name');
    
    let foundName = null;
    let logs = [];

    if (isCloudActive) {
        const snapshot = await db.ref("subjects").once("value");
        snapshot.forEach(child => { 
            if (child.key.toLowerCase() === query) { 
                foundName = child.key; 
                logs = Object.values(child.val()); 
            } 
        });
    } else {
        const subjects = JSON.parse(localStorage.getItem('wash_rp_subjects') || '{}');
        foundName = Object.keys(subjects).find(n => n.toLowerCase() === query);
        if (foundName) logs = subjects[foundName];
    }

    if (foundName) {
        title.innerText = `Cartella: ${foundName}`;
        view.style.display = 'block';
        renderAdminSancLists(foundName, logs);
    } else {
        showModal("Errore", "Soggetto non trovato nel database.", null);
        view.style.display = 'none';
    }
}

function renderAdminSancLists(name, logs) {
    const warnList = document.getElementById('admin-warn-list');
    const banList = document.getElementById('admin-ban-list');
    const warnDiscordList = document.getElementById('admin-warn-discord-list');
    
    warnList.innerHTML = ''; banList.innerHTML = ''; warnDiscordList.innerHTML = '';

    logs.forEach(log => {
        const item = `
            <div class="status-item-small" style="margin-bottom: 10px; padding: 10px; background: rgba(255,255,255,0.03); border-radius: 8px; display: flex; align-items: center; gap: 12px;">
                <img src="${log.subjectAvatar || 'assets/logo.png'}" style="width: 35px; height: 35px; border-radius: 50%; border: 1px solid var(--accent-purple); object-fit: cover;">
                <div style="flex: 1; font-size: 0.8rem;">
                    <strong>${new Date(log.date).toLocaleDateString()}</strong><br>
                    ${log.reason}<br>
                    <small>Da: ${log.signature || log.staff}</small>
                </div>
                <i class="fas fa-trash-alt btn-remove" onclick="confirmRemoveSanc('${name}', '${log.id}')"></i>
            </div>
        `;
        if (log.type === 'warn') warnList.innerHTML += item;
        else if (log.type === 'ban') banList.innerHTML += item;
        else if (log.type === 'warn-discord') warnDiscordList.innerHTML += item;
    });
}

function confirmRemoveSanc(name, logId) {
    showModal("Rimuovi Sanzione", `Sei sicuro di voler eliminare questa sanzione per ${name}?`, async () => {
        if (isCloudActive) {
            await db.ref(`subjects/${name}/${logId}`).remove();
        } else {
            let subjects = JSON.parse(localStorage.getItem('wash_rp_subjects') || '{}');
            subjects[name] = subjects[name].filter(l => l.id != logId);
            localStorage.setItem('wash_rp_subjects', JSON.stringify(subjects));
        }
        adminSearchSubject(); // Refresh view
        renderActiveBans();
        addGlobalLog(`Sanzione rimossa a ${name} da un amministratore.`, 'sanctions');
    });
}

// --- AUTHENTICATION ---
function logout() {
    if (currentUser) addGlobalLog(`Staff ${currentUser.name} ha effettuato il logout.`, 'access');
    localStorage.removeItem('wash_rp_session');
    sessionStorage.removeItem('discord_access_token');
    sessionStorage.removeItem('staff_auth');
    location.reload();
}

async function checkAdminLogin() {
    const btn = document.querySelector('#admin-lock .btn-start');
    const passInput = document.getElementById('admin-pass-input');
    const pass = passInput.value;
    let correctPass = "WSHT_rp2026";

    if (btn) btn.innerText = "VERIFICA...";

    try {
        if (isCloudActive) {
            const snap = await db.ref("config/passwords/admin").once("value");
            if (snap.exists()) correctPass = snap.val();
        }

        if (pass === correctPass) {
            sessionStorage.setItem('admin_auth', 'true');
            document.getElementById('admin-lock').style.display = 'none';
            document.getElementById('admin-content').style.display = 'block';
            passInput.value = '';
            renderGlobalLogs();
            renderInactivityRequests();
            addGlobalLog(`${currentUser.name} ha effettuato l'accesso all'Area Amministrativa.`, 'access');
        } else {
            showModal("Accesso Negato", "Password Amministratore Errata!", null);
        }
    } catch (e) {
        console.error(e);
        alert("Errore di connessione al database admin.");
    } finally {
        if (btn) btn.innerText = "SBLOCCA";
    }
}

// Alias per compatibilità HTML
function checkAdminPass() { checkAdminLogin(); }

async function checkStaffLogin() {
    const user = document.getElementById('login-user').value.trim();
    const pass = document.getElementById('login-pass').value;
    const btn = document.querySelector('.login-btn');
    const errorEl = document.getElementById('login-error');
    
    if (!user.startsWith('@')) {
        showModal("Attenzione", "Il nome utente deve iniziare obbligatoriamente con la @ per accedere!", null, false, true);
        return;
    }

    if (btn) { btn.disabled = true; btn.innerText = "VERIFICA ACCESSO..."; }
    if (errorEl) errorEl.style.display = 'none';

    let correctPass = "Washinton_staff2026";

    try {
        if (isCloudActive) {
            const snap = await db.ref("config/passwords/staff").once("value");
            if (snap.exists()) correctPass = snap.val();
        }

        // Controllo autorizzazione
        let isAuthorized = (user.toLowerCase() === DEFAULT_ADMIN.toLowerCase());
        
        if (!isAuthorized && isCloudActive) {
            // Firebase non accetta punti nei nomi delle chiavi, li sostituiamo con underscore per il controllo
            const safeNick = user.replace(/\./g, '_');
            const authSnap = await db.ref(`authorized_staff/${safeNick}`).once("value");
            if (authSnap.exists()) isAuthorized = true;
        }

        if (pass === correctPass && isAuthorized) {
            let staffRank = "Staff";
            if (isCloudActive) {
                const safeNick = user.replace(/\./g, '_');
                const authSnap = await db.ref(`authorized_staff/${safeNick}`).once("value");
                if (authSnap.exists()) staffRank = authSnap.val().rank;
            }

            currentUser = { 
                name: user, 
                pfp: discordTempAvatar || "assets/logo.png",
                rank: staffRank
            };
            localStorage.setItem('wash_rp_session', JSON.stringify(currentUser));
            showDashboard();
            loadUserServiceTime();
            updateNotifUI();
            addGlobalLog(`Staff ${user} ha effettuato l'accesso.`, 'access');
        } else {
            if (errorEl) {
                errorEl.innerText = "Attenzione: il tuo nome staff o la password non sono corretti!";
                errorEl.style.display = 'block';
            }
        }
    } catch (e) {
        console.error(e);
        alert("Errore critico durante l'accesso. Controlla la connessione.");
    } finally {
        if (btn) { btn.disabled = false; btn.innerText = "ACCEDI"; }
    }
}

function showDashboard() {
    // Se currentUser è già popolato (da checkSession o fetchDiscordUser), non sovrascriviamolo con dati vuoti
    if (!currentUser || !currentUser.name || currentUser.name === 'Staff Name') {
        const nickInput = document.getElementById('login-user');
        const nick = nickInput ? nickInput.value : '';
        
        currentUser = { 
            name: nick || document.getElementById('nav-username').innerText || "Staff", 
            pfp: discordTempAvatar || "assets/logo.png",
            displayName: nick || "Staff"
        };
    }

    const overlay = document.getElementById('login-overlay');
    const dashboard = document.getElementById('dashboard');
    
    if (overlay) overlay.style.opacity = '0';
    
    setTimeout(() => {
        if (overlay) overlay.style.display = 'none';
        if (dashboard) dashboard.style.display = 'flex';
        
        // Aggiorniamo l'interfaccia con i dati reali
        const navName = document.getElementById('nav-username');
        const navPfp = document.getElementById('nav-pfp');
        
        if (navName) navName.innerText = currentUser.displayName || currentUser.name;
        if (navPfp && currentUser.pfp) navPfp.src = currentUser.pfp;
    }, 500);
}

// --- STAFF SERVICE ---
async function startService() {
    if (serviceInterval) return;
    const btn = document.querySelector('.btn-start');
    if (btn) btn.disabled = true;

    try {
        serviceInterval = setInterval(updateTimer, 1000);
        const startTime = new Date().toLocaleTimeString();
        if (isCloudActive) {
            await db.ref(`staff/${currentUser.name}`).update({ 
                status: "In Servizio", 
                startTime: startTime,
                lastUpdate: new Date().toISOString() 
            });
        } else {
            let staffStatus = JSON.parse(localStorage.getItem('wash_rp_staff_status') || '{}');
            staffStatus[currentUser.name] = { status: "In Servizio", startTime: startTime };
            localStorage.setItem('wash_rp_staff_status', JSON.stringify(staffStatus));
        }
        addGlobalLog(`Staff ${currentUser.name} ha avviato il servizio.`, 'staff-service');
    } catch (e) {
        console.error(e);
        clearInterval(serviceInterval);
        serviceInterval = null;
        alert("Errore nell'avvio del servizio.");
    } finally {
        if (btn) btn.disabled = false;
    }
}


async function pauseService() {
    if (!serviceInterval) return;
    clearInterval(serviceInterval);
    serviceInterval = null;
    
    if (isCloudActive) {
        await db.ref(`staff/${currentUser.name}`).update({ status: "In Pausa" });
    }
    
    addGlobalLog(`Staff ${currentUser.name} ha messo in pausa il servizio.`, 'staff-service');
    saveCurrentTime();
}

async function stopService() {
    if (serviceInterval) clearInterval(serviceInterval);
    serviceInterval = null;
    
    if (isCloudActive) {
        await db.ref(`staff/${currentUser.name}`).update({ 
            status: "Offline", 
            startTime: null 
        });
    } else {
        let staffStatus = JSON.parse(localStorage.getItem('wash_rp_staff_status') || '{}');
        staffStatus[currentUser.name] = { status: "Offline", startTime: null };
        localStorage.setItem('wash_rp_staff_status', JSON.stringify(staffStatus));
    }

    saveCurrentTime();
    addGlobalLog(`Staff ${currentUser.name} ha terminato il servizio.`, 'staff-service');
    showModal("Servizio Terminato", "Il tuo tempo è stato salvato correttamente. Non è stato resettato, potrai riprendere da qui.", null);
}

async function saveCurrentTime() {
    if (!currentUser) return;
    if (isCloudActive) {
        await cloudSave(`staff/${currentUser.name}`, { hours: totalActiveMs, lastUpdate: new Date().toISOString() });
    } else {
        let staffData = JSON.parse(localStorage.getItem('wash_rp_staff_hours') || '{}');
        staffData[currentUser.name] = totalActiveMs;
        localStorage.setItem('wash_rp_staff_hours', JSON.stringify(staffData));
    }
    updateStaffList();
}

function updateTimer() {
    totalActiveMs += 1000;
    updateTimerUI(totalActiveMs);
    if (totalActiveMs % 60000 === 0) saveCurrentTime();
}
function updateTimerUI(ms) {
    const h = Math.floor(ms / (1000 * 60 * 60));
    const m = Math.floor((ms % (1000 * 60 * 60)) / (1000 * 60));
    const s = Math.floor((ms % (1000 * 60)) / 1000);
    document.getElementById('service-timer').innerText = `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`;
    const progress = Math.min((ms / CONFIG.SERVICE_GOAL_MS) * 100, 100);
    const bar = document.getElementById('service-progress');
    if (bar) bar.style.width = `${progress}%`;
    const goalText = document.querySelector('.progress-container + p');
    if (goalText) goalText.innerText = `Obiettivo: 4 Ore (${Math.floor((ms / CONFIG.SERVICE_GOAL_MS) * 100)}%)`;
}

function changeRankingPage(delta) {
    currentRankingPage += delta;
    if (currentRankingPage < 0) currentRankingPage = 0;
    updateStaffList();
}

async function updateStaffList() {
    const activeBody = document.getElementById('active-staff-body');
    const rankingBody = document.getElementById('hours-ranking-body');
    if (!activeBody || !rankingBody) return;

    let staffData = {};
    let authorizedData = {};
    if (isCloudActive) {
        const snapshot = await db.ref("staff").once("value");
        if (snapshot.exists()) staffData = snapshot.val();
        const authSnap = await db.ref("authorized_staff").once("value");
        if (authSnap.exists()) authorizedData = authSnap.val();
    } else {
        const hours = JSON.parse(localStorage.getItem('wash_rp_staff_hours') || '{}');
        const status = JSON.parse(localStorage.getItem('wash_rp_staff_status') || '{}');
        Object.keys(hours).forEach(name => {
            staffData[name] = { 
                hours: hours[name], 
                status: status[name]?.status || "Offline", 
                startTime: status[name]?.startTime || "" 
            };
        });
    }

    // --- RENDER ACTIVE STAFF ---
    activeBody.innerHTML = '';
    const activeStaff = Object.keys(staffData).filter(name => staffData[name].status === "In Servizio");
    if (activeStaff.length === 0) {
        activeBody.innerHTML = '<tr><td colspan="3" style="text-align:center; opacity:0.5;">Nessuno in servizio</td></tr>';
    } else {
        activeStaff.forEach(name => {
            const safeNick = name.replace(/\./g, '_');
            const authInfo = authorizedData[safeNick] || {};
            const avatar = authInfo.avatar || 'assets/logo.png';
            const rank = authInfo.rank || 'Staff';
            
            activeBody.innerHTML += `
                <tr>
                    <td style="display: flex; align-items: center; gap: 10px;">
                        <img src="${avatar}" style="width: 30px; height: 30px; border-radius: 50%; border: 2px solid var(--accent-purple); object-fit: cover;">
                        <div style="display: flex; flex-direction: column;">
                            <strong style="font-size: 0.85rem;">${name}</strong>
                            <small style="opacity: 0.6; font-size: 0.7rem;">${rank}</small>
                        </div>
                    </td>
                    <td>${staffData[name].startTime || "---"}</td>
                    <td><span class="status-tag" style="background: rgba(46,204,113,0.2); color: #2ecc71; border: 1px solid #2ecc71;">ATTIVO</span></td>
                </tr>
            `;
        });
    }

    // --- RENDER HOURS RANKING (Sorted by Hours & Paginated) ---
    rankingBody.innerHTML = '';
    
    // Uniamo i dati: prendiamo solo gli staff autorizzati e associamo le loro ore
    const rankingArray = Object.values(authorizedData).map(auth => {
        const sData = staffData[auth.nick] || { hours: 0 };
        return {
            nick: auth.nick,
            rank: auth.rank,
            avatar: auth.avatar || 'assets/logo.png',
            hours: sData.hours || 0
        };
    }).sort((a, b) => b.hours - a.hours); // Decrescente per ore

    const totalPages = Math.ceil(rankingArray.length / RANKING_PAGE_SIZE);
    if (currentRankingPage >= totalPages && totalPages > 0) currentRankingPage = totalPages - 1;

    const startIdx = currentRankingPage * RANKING_PAGE_SIZE;
    const endIdx = startIdx + RANKING_PAGE_SIZE;
    const pageItems = rankingArray.slice(startIdx, endIdx);

    document.getElementById('ranking-page-info').innerText = `Pagina ${currentRankingPage + 1} di ${Math.max(1, totalPages)}`;

    if (rankingArray.length === 0) {
        rankingBody.innerHTML = '<tr><td colspan="4" style="text-align:center; opacity:0.5;">Nessun dato</td></tr>';
    } else {
        pageItems.forEach((item, index) => {
            const globalIndex = startIdx + index;
            const ms = item.hours;
            const h = Math.floor(ms / (1000 * 60 * 60));
            const m = Math.floor((ms % (1000 * 60 * 60)) / (1000 * 60));
            const isGoalMet = ms >= CONFIG.SERVICE_GOAL_MS;
            
            rankingBody.innerHTML += `
                <tr style="border-bottom: 1px solid rgba(255,255,255,0.03);">
                    <td style="opacity: 0.5; font-weight: 700; padding: 15px 10px;">#${globalIndex + 1}</td>
                    <td style="display: flex; align-items: center; gap: 12px; padding: 12px 10px;">
                        <img src="${item.avatar}" style="width: 35px; height: 35px; border-radius: 8px; border: 1px solid var(--accent-purple); object-fit: cover;">
                        <div style="display: flex; flex-direction: column;">
                            <strong style="font-size: 0.85rem; color: #fff;">${item.nick}</strong>
                            <small style="opacity: 0.5; font-size: 0.7rem;">${item.rank}</small>
                        </div>
                    </td>
                    <td style="font-weight: 600; color: var(--accent-purple);">${h}h ${m}m</td>
                    <td>${isGoalMet ? '<i class="fas fa-check-circle" style="color: #2ecc71;"></i>' : '<i class="fas fa-times-circle" style="color: var(--accent-red);"></i>'}</td>
                </tr>
            `;
        });
    }
}

// --- SANCTIONS ---
async function submitSanc() {
    const btn = document.querySelector('#sanzioni .login-btn');
    const type = document.getElementById('sanc-type').value;
    const subject = document.getElementById('sanc-subject').value.trim();
    const reason = document.getElementById('sanc-reason').value.trim();
    const expiry = document.getElementById('sanc-expiry').value;
    const signature = document.getElementById('sanc-signature').value.trim();

    if (!subject || !reason || !signature) { 
        showModal("Campo Mancante", "Assicurati di aver compilato il Nome Soggetto, la Motivazione e la Firma.", null); 
        return; 
    }
    
    if (!signature.startsWith('@')) { 
        showModal("Firma Non Valida", "La firma deve obbligatoriamente iniziare con il simbolo @ (es. @TuoNome).", null); 
        return; 
    }

    if (btn) { 
        btn.disabled = true; 
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> INVIO IN CORSO...'; 
    }

    // Sicurezza: assicuriamoci che currentUser sia popolato
    if (!currentUser) {
        currentUser = JSON.parse(localStorage.getItem('wash_rp_session') || '{"name":"Staff","displayName":"Staff"}');
    }

    const log = { 
        id: Date.now(), 
        type, 
        subject, 
        subjectAvatar: selectedRobloxAvatar || "assets/logo.png",
        staff: currentUser.name || "Staff", 
        reason, 
        signature, 
        date: new Date().toISOString(), 
        expiry: expiry || null 
    };
    
    try {
        if (isCloudActive) {
            await db.ref(`subjects/${subject}/${log.id}`).set(log);
        } else {
            let subjects = JSON.parse(localStorage.getItem('wash_rp_subjects') || '{}');
            if (!subjects[subject]) subjects[subject] = [];
            subjects[subject].push(log);
            localStorage.setItem('wash_rp_subjects', JSON.stringify(subjects));
        }

        addGlobalLog(`${currentUser.name} ha applicato un ${type.toUpperCase()} a ${subject} [Firma: ${signature}]`, 'sanctions');
        addNotification(subject, `Hai ricevuto un ${type.toUpperCase()}. Motivo: ${reason}. Firmato: ${signature}`);
        
        // Feedback Successo
        showModal("✅ Operazione Riuscita", `La sanzione (${type.toUpperCase()}) per <b>${subject}</b> è stata registrata correttamente.`, () => closeModal(), false, true);
        
        setTimeout(() => {
            const confirmBtn = document.getElementById('modal-confirm-btn');
            if (confirmBtn) confirmBtn.innerText = "CONFERMA";
        }, 50);
        
        // Pulizia Form
        ['sanc-subject', 'sanc-reason', 'sanc-expiry'].forEach(id => { 
            const el = document.getElementById(id); 
            if (el) el.value = ''; 
        });
        const prev = document.getElementById('sanc-roblox-preview');
        if (prev) prev.style.display = 'none';
        
        renderActiveBans();
    } catch (e) {
        console.error(e);
        showModal("Errore", "Si è verificato un errore durante l'invio della sanzione.", null);
    } finally {
        if (btn) { 
            btn.disabled = false; 
            btn.innerText = type === 'warn' ? "INVIA WARN" : (type === 'ban' ? "INVIA BAN" : "INVIA SANZIONE"); 
        }
    }
}


async function renderActiveBans() {
    const container = document.getElementById('active-bans-container');
    if (!container) return;
    let subjects = {};
    if (isCloudActive) {
        const snapshot = await db.ref("subjects").once("value");
        if (snapshot.exists()) subjects = snapshot.val();
    } else {
        subjects = JSON.parse(localStorage.getItem('wash_rp_subjects') || '{}');
    }
    const now = new Date();
    let html = '';
    Object.keys(subjects).forEach(name => {
        const logs = Array.isArray(subjects[name]) ? subjects[name] : Object.values(subjects[name]);
        logs.forEach(log => {
            if (log.type === 'ban' && log.expiry) {
                const isActive = new Date(log.expiry) > now;
                html += `<div class="log-entry" style="display: flex; justify-content: space-between; align-items: center;"><div><strong>${name}</strong> - Motivo: ${log.reason}<br><small>Firma: ${log.signature} | Scadenza: ${new Date(log.expiry).toLocaleDateString()}</small></div><span class="status-tag ${isActive ? 'status-ban-active' : 'status-ban-ended'}">${isActive ? 'BAN IN CORSO' : 'BAN TERMINATO'}</span></div>`;
            }
        });
    });
    container.innerHTML = html || '<p style="opacity: 0.5;">Nessun ban attivo trovato.</p>';
}

// --- SEARCH ---
async function searchSubject() {
    const query = document.getElementById('search-input').value.trim().toLowerCase();
    const results = document.getElementById('search-results');
    const btn = document.querySelector('#search .btn-start');
    
    if (!query) {
        showModal("Attenzione", "Inserisci un nickname o ID per cercare.", null);
        return;
    }

    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> RICERCA...';
    }

    results.innerHTML = '';
    let foundName = null;
    let logs = [];

    try {
        if (isCloudActive) {
            const snapshot = await db.ref("subjects").once("value");
            snapshot.forEach(child => { 
                if (child.key.toLowerCase() === query) { 
                    foundName = child.key; 
                    logs = Object.values(child.val()); 
                } 
            });
        } else {
            const subjects = JSON.parse(localStorage.getItem('wash_rp_subjects') || '{}');
            foundName = Object.keys(subjects).find(name => name.toLowerCase() === query);
            if (foundName) logs = subjects[foundName];
        }

        if (foundName) {
            const lastLog = logs[logs.length - 1];
            const avatar = lastLog.subjectAvatar || "assets/logo.png";
            
            results.innerHTML = `
                <div class="search-header" style="display: flex; align-items: center; gap: 20px; margin-top: 2rem; padding: 25px; background: rgba(123, 44, 191, 0.05); border-radius: 15px; border: 1px solid rgba(123, 44, 191, 0.3); box-shadow: 0 4px 20px rgba(0,0,0,0.2);">
                    <img src="${avatar}" style="width: 70px; height: 70px; border-radius: 12px; border: 2px solid var(--accent-purple); object-fit: cover;">
                    <div>
                        <h2 style="margin: 0; color: white; font-size: 1.4rem; letter-spacing: 0.5px;">Soggetto trovato: <span style="color: var(--accent-purple);">${foundName}</span></h2>
                        <p style="margin: 5px 0 0 0; opacity: 0.6; font-size: 0.85rem; font-weight: 500;">Anagrafica sanzioni caricata correttamente.</p>
                    </div>
                </div>
                <h3 style="margin-top: 2.5rem; font-size: 1.1rem; opacity: 0.9;"><i class="fas fa-list-ul" style="margin-right: 10px; color: var(--accent-purple);"></i> Cronologia Provvedimenti</h3>
                <div style="width: 100%; height: 1px; background: linear-gradient(to right, var(--accent-purple), transparent); margin: 1rem 0; opacity: 0.3;"></div>`;

            logs.slice().reverse().forEach(log => {
                const typeColor = log.type === 'ban' ? 'var(--accent-red)' : (log.type === 'warn' ? '#f1c40f' : 'var(--accent-purple)');
                results.innerHTML += `
                    <div class="log-entry" style="border-left: 3px solid ${typeColor}; background: rgba(255,255,255,0.03); margin-bottom: 12px; padding: 18px; border-radius: 8px; transition: transform 0.2s;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <div>
                                <strong style="color: ${typeColor}; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 1px;">${log.type}</strong>
                                <br>
                                <div style="margin-top: 8px; color: rgba(255,255,255,0.9); font-size: 0.95rem;">${log.reason}</div>
                            </div>
                            <div style="text-align: right;">
                                <small style="opacity: 0.4; font-size: 0.7rem; display: block;">${new Date(log.date).toLocaleDateString()} ${new Date(log.date).toLocaleTimeString()}</small>
                                <small style="font-style: italic; opacity: 0.6; font-size: 0.75rem; margin-top: 5px; display: block;">Firma: ${log.signature}</small>
                            </div>
                        </div>
                    </div>`;
            });
        } else {
            results.innerHTML = `
                <div style="text-align: center; margin-top: 3rem; opacity: 0.5;">
                    <i class="fas fa-user-slash" style="font-size: 3rem; margin-bottom: 1rem;"></i>
                    <p>Nessun soggetto trovato nel database delle sanzioni.</p>
                </div>`;
        }
    } catch (e) {
        console.error("Errore ricerca:", e);
        showModal("Errore", "Si è verificato un errore durante la ricerca del soggetto.", null);
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-search"></i> Cerca';
        }
    }
}

// --- INACTIVITY ---
async function submitInactivity() {
    const btn = document.querySelector('#inattivita .login-btn');
    const discord = document.getElementById('ina-discord').value.trim();
    const rank = document.getElementById('ina-rank').value.trim();
    const reason = document.getElementById('ina-reason').value.trim();
    const start = document.getElementById('ina-start').value;
    const end = document.getElementById('ina-end').value;

    if (!discord || !rank || !reason || !start || !end) { showModal("Errore", "Compila tutti i campi!", null); return; }
    if (!discord.startsWith('@')) { showModal("Errore", "Il nome Discord deve iniziare obbligatoriamente con @!", null); return; }
    if (btn) { btn.disabled = true; btn.innerText = "INVIO IN CORSO..."; }

    const request = { id: Date.now(), user: discord, rank, reason, start, end, status: 'pending' };

    try {
        if (isCloudActive) {
            await db.ref(`inactivity/${request.id}`).set(request);
        } else {
            let requests = JSON.parse(localStorage.getItem('wash_rp_inactivity') || '[]');
            requests.push(request);
            localStorage.setItem('wash_rp_inactivity', JSON.stringify(requests));
        }

        showModal("Successo", "Richiesta inviata con successo!", null);
        ['ina-discord', 'ina-rank', 'ina-reason', 'ina-start', 'ina-end'].forEach(id => document.getElementById(id).value = '');
        renderInactivityRequests();
    } catch (e) {
        console.error(e);
        alert("Errore nell'invio della richiesta.");
    } finally {
        if (btn) { btn.disabled = false; btn.innerText = "INVIA RICHIESTA"; }
    }
}


async function renderInactivityRequests() {
    const list = document.getElementById('admin-inactivity-list');
    if (!list) return;
    let requests = [];
    if (isCloudActive) {
        const snapshot = await db.ref("inactivity").once("value");
        if (snapshot.exists()) {
            snapshot.forEach(child => { if (child.val().status === 'pending') requests.push(child.val()); });
        }
    } else {
        requests = JSON.parse(localStorage.getItem('wash_rp_inactivity') || '[]').filter(r => r.status === 'pending');
    }
    if (requests.length === 0) { list.innerHTML = '<p style="opacity: 0.5;">Nessuna richiesta pendente.</p>'; return; }
    list.innerHTML = requests.map(r => `
        <div class="request-card">
            <strong>${r.user}</strong> (${r.rank})<br>
            <p style="font-size:0.85rem; margin: 0.5rem 0;">Motivo: ${r.reason}<br>Periodo: ${r.start} / ${r.end}</p>
            <div style="display:flex; gap:10px;">
                <button class="btn btn-start" onclick="handleInactivity('${r.id}', 'approved')">ACCETTA</button>
                <button class="btn btn-stop" onclick="handleInactivity('${r.id}', 'rejected')">RIFIUTA</button>
            </div>
        </div>
    `).join('');
}

async function handleInactivity(id, status) {
    if (status === 'rejected') {
        showModal("Rifiuta Richiesta", "Inserisci il motivo del rifiuto:", (reason) => { processInactivity(id, 'rejected', reason); }, true);
    } else { processInactivity(id, 'approved'); }
}

async function processInactivity(id, status, reason = "") {
    let targetReq = null;
    if (isCloudActive) {
        const snapshot = await db.ref(`inactivity/${id}`).once("value");
        if (snapshot.exists()) targetReq = snapshot.val();
    } else {
        let requests = JSON.parse(localStorage.getItem('wash_rp_inactivity') || '[]');
        targetReq = requests.find(r => r.id == id);
    }
    if (!targetReq) return;

    targetReq.status = status;
    const userName = targetReq.user;
    
    if (isCloudActive) {
        await db.ref(`inactivity/${id}`).update({ status: status });
    } else {
        let requests = JSON.parse(localStorage.getItem('wash_rp_inactivity') || '[]');
        requests.find(r => r.id == id).status = status;
        localStorage.setItem('wash_rp_inactivity', JSON.stringify(requests));
    }
    
    const msg = status === 'approved' ? "Richiesta inattività ACCETTATA. Buon riposo!" : `Richiesta inattività RIFIUTATA. Motivo: ${reason}`;
    addNotification(userName, msg);
    
    const adminNick = currentUser ? currentUser.name : "Admin";
    const adminRank = currentUser ? currentUser.rank : "Amministratore";
    const adminFull = `${adminNick} (${adminRank})`;

    if (isCloudActive) {
        await db.ref(`inactivity/${id}`).update({ acceptedBy: adminFull });
    }

    addGlobalLog(`${adminNick} ha gestito la richiesta di ${userName} come ${status === 'approved' ? 'ACCETTATA' : 'RIFIUTATA'}`, 'access');
    renderInactivityRequests();
    renderActiveInactivityList();
    showModal("Successo", `Richiesta ${status === 'approved' ? 'accettata' : 'rifiutata'}!`, null);
}
async function renderActiveInactivityList() {
    const grid = document.getElementById('active-inactivity-grid');
    if (!grid) return;
    
    let activeInactivities = [];
    if (isCloudActive) {
        const snapshot = await db.ref("inactivity").once("value");
        if (snapshot.exists()) {
            const now = new Date();
            snapshot.forEach(child => {
                const r = child.val();
                if (r.status === 'approved' && new Date(r.end) >= now) {
                    activeInactivities.push(r);
                }
            });
        }
    }

    if (activeInactivities.length === 0) {
        grid.innerHTML = '<p style="opacity: 0.5; text-align: center; width: 100%; grid-column: 1/-1;">Nessuna inattività attiva al momento.</p>';
        return;
    }

    grid.innerHTML = activeInactivities.map(r => {
        const isAdmin = sessionStorage.getItem('admin_auth') === 'true';
        const deleteBtn = isAdmin ? `<i class="fas fa-trash-alt" style="color: var(--accent-red); cursor: pointer; font-size: 0.8rem; margin-left: 10px;" onclick="confirmRemoveInactivity('${r.id}', '${r.user}')" title="Rimuovi Inattività"></i>` : '';

        return `
            <div class="status-square" style="min-height: auto; padding: 20px; border-left: 4px solid #f1c40f;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 5px;">
                    <div style="display: flex; align-items: center;">
                        <strong style="font-size: 1.1rem; color: #fff;">${r.user}</strong>
                        ${deleteBtn}
                    </div>
                    <span class="status-tag" style="background: rgba(241, 196, 15, 0.2); color: #f1c40f; border: 1px solid #f1c40f; font-weight: 800; font-size: 0.65rem;">INATTIVO</span>
                </div>
                <div style="font-size: 0.75rem; color: var(--accent-purple); font-weight: 600; margin-bottom: 12px; opacity: 0.8;">
                    <i class="fas fa-id-badge"></i> Grado: ${r.rank || 'Staff'}
                </div>
                <div style="font-size: 0.85rem; opacity: 0.7; margin-bottom: 10px;">
                    <i class="fas fa-calendar-alt"></i> ${new Date(r.start).toLocaleDateString()} - ${new Date(r.end).toLocaleDateString()}
                </div>
                <div style="font-size: 0.9rem; margin-bottom: 15px; color: rgba(255,255,255,0.9);">
                    <strong>Motivo:</strong> ${r.reason}
                </div>
                <div style="font-size: 0.75rem; opacity: 0.5; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 10px;">
                    Accettata da: <span style="color: var(--accent-purple); font-weight: 600;">${r.acceptedBy || 'Admin'}</span>
                </div>
            </div>
        `;
    }).join('');
}
// --- STRIKES ---
async function submitStrike() {
    const btn = document.querySelector('#admin .login-btn');
    const target = document.getElementById('strike-target').value.trim();
    const reason = document.getElementById('strike-reason').value.trim();
    const num = document.getElementById('strike-num').value;
    const signature = document.getElementById('strike-signature').value.trim();

    if (!target || !reason || !signature) { showModal("Errore", "Compila tutti i campi!", null); return; }
    if (btn) { btn.disabled = true; btn.innerText = "INVIO IN CORSO..."; }

    const strike = { id: Date.now(), target, reason, num, signature, date: new Date().toISOString() };

    try {
        if (isCloudActive) {
            await db.ref(`strikes/${target}/${strike.id}`).set(strike);
        } else {
            let strikes = JSON.parse(localStorage.getItem('wash_rp_strikes') || '{}');
            if (!strikes[target]) strikes[target] = [];
            strikes[target].push(strike);
            localStorage.setItem('wash_rp_strikes', JSON.stringify(strikes));
        }

        addNotification(target, `Hai ricevuto uno STRIKE (${num}). Motivo: ${reason}. Firma: ${signature}`);
        addGlobalLog(`${currentUser.name} ha assegnato uno STRIKE (${num}) a ${target} [Firma: ${signature}]`, 'sanctions');
        showModal("Successo", "Strike assegnato con successo!", null);
        ['strike-target', 'strike-reason', 'strike-signature'].forEach(id => document.getElementById(id).value = '');
    } catch (e) {
        console.error(e);
        alert("Errore nell'assegnazione dello strike.");
    } finally {
        if (btn) { btn.disabled = false; btn.innerText = "AGGIUNGI STRIKE"; }
    }
}


// --- STAFF CONTROL ---
async function checkStaffStatus() {
    const input = document.getElementById('staff-check-input').value.trim();
    const query = input.toLowerCase();
    const result = document.getElementById('staff-status-result');
    
    if (!input.startsWith('@')) {
        showModal("Errore", "Il nome staff deve iniziare obbligatoriamente con <b>@</b>.", null);
        return;
    }

    let authorizedData = {};
    let staffHours = {};
    let strikes = {};
    let inactivity = [];

    if (isCloudActive) {
        const authSnap = await db.ref("authorized_staff").once("value");
        if (authSnap.exists()) authorizedData = authSnap.val();
        
        const hoursSnap = await db.ref("staff").once("value");
        if (hoursSnap.exists()) staffHours = hoursSnap.val();
        
        const strikesSnap = await db.ref("strikes").once("value");
        if (strikesSnap.exists()) strikes = strikesSnap.val();
        
        const inaSnap = await db.ref("inactivity").once("value");
        if (inaSnap.exists()) inaSnap.forEach(child => inactivity.push(child.val()));
    }

    // Verifica se lo staffer esiste davvero
    const foundAuth = Object.values(authorizedData).find(a => a.nick.toLowerCase() === query);
    
    if (!foundAuth) {
        result.innerHTML = `
            <div style="text-align: center; padding: 2rem; background: rgba(230, 57, 70, 0.1); border-radius: 12px; border: 1px solid var(--accent-red); margin-top: 1rem;">
                <i class="fas fa-user-times" style="font-size: 2rem; color: var(--accent-red); margin-bottom: 10px;"></i>
                <p style="color: #fff; font-weight: 600;">Il soggetto non fa parte dello staff.</p>
                <p style="opacity: 0.7; font-size: 0.8rem;">Controllare che il nick sia corretto e includa la @.</p>
            </div>
        `;
        return;
    }

    const target = foundAuth.nick;
    const ms = (staffHours[target]?.hours) || 0;
    const userStrikes = strikes[target] ? Object.values(strikes[target]) : [];
    const userInactivity = inactivity.filter(r => r.user.toLowerCase() === query);

    result.innerHTML = `
        <div class="status-grid">
            <div class="status-square">
                <h4>💼 Inattività</h4>
                ${userInactivity.length === 0 ? '<p style="opacity:0.5; font-size:0.8rem;">Nessuna richiesta.</p>' : userInactivity.map(r => `
                    <div class="status-item-small">
                        <div>${r.start} / ${r.end}<br><small>${r.status}</small></div>
                        <i class="fas fa-times-circle btn-remove" onclick="confirmRemoveInactivity('${r.id}', '${target}')"></i>
                    </div>
                `).join('')}
            </div>
            <div class="status-square">
                <h4>🕒 Ore Totali</h4>
                <div style="text-align:center; margin-top:2rem;">
                    <div style="font-size:2rem; font-weight:700;">${Math.floor(ms/(1000*60*60))}h</div>
                    <div style="opacity:0.6;">${Math.floor((ms%(1000*60*60))/(1000*60))}m totali</div>
                </div>
            </div>
            <div class="status-square">
                <h4>⚠️ Strikes</h4>
                ${userStrikes.length === 0 ? '<p style="opacity:0.5; font-size:0.8rem;">Nessuno strike.</p>' : userStrikes.map(s => `
                    <div class="status-item-small">
                        <div>Strike ${s.num}<br><small>${s.reason}</small></div>
                        <i class="fas fa-times-circle btn-remove" onclick="confirmRemoveStrike('${s.id}', '${target}')"></i>
                    </div>
                `).join('')}
            </div>
        </div>
    `;
}

function confirmRemoveInactivity(id, user) {
    showModal("Rimuovi Inattività", "Sei sicuro di voler eliminare questa richiesta?", async () => {
        if (isCloudActive) await db.ref(`inactivity/${id}`).remove();
        else {
            let requests = JSON.parse(localStorage.getItem('wash_rp_inactivity') || '[]');
            requests = requests.filter(r => r.id != id);
            localStorage.setItem('wash_rp_inactivity', JSON.stringify(requests));
        }
        addGlobalLog(`${currentUser.name} ha rimosso una richiesta inattività di ${user} [Firma: ${currentUser.name}]`, 'access');
        checkStaffStatus();
        renderActiveInactivityList();
    });
}

function confirmRemoveStrike(id, user) {
    showModal("Rimuovi Strike", "Sei sicuro di voler togliere questo strike?", async () => {
        if (isCloudActive) {
            await db.ref(`strikes/${user}/${id}`).remove();
        } else {
            let strikes = JSON.parse(localStorage.getItem('wash_rp_strikes') || '{}');
            strikes[user] = strikes[user].filter(s => s.id != id);
            localStorage.setItem('wash_rp_strikes', JSON.stringify(strikes));
        }
        addGlobalLog(`${currentUser.name} ha rimosso uno strike a ${user} [Firma: ${currentUser.name}]`, 'sanctions');
        checkStaffStatus();
    });
}

// --- GESTIONE PERSONALE STAFF (ADMIN) ---
async function registerStaff() {
    const date = document.getElementById('reg-staff-date').value;
    const nick = document.getElementById('reg-staff-nick').value.trim();
    const rank = document.getElementById('reg-staff-rank').value;

    if (!date || !nick || !rank) { 
        showModal("Errore", "Devi compilare tutti i campi per registrare lo staff!", null); 
        return; 
    }
    if (!nick.startsWith('@')) { 
        showModal("Errore", "Il nickname deve iniziare con @!", null); 
        return; 
    }

    try {
        if (isCloudActive) {
            const safeNick = nick.replace(/\./g, '_');
            
            // CONTROLLO DUPLICATI
            const snap = await db.ref(`authorized_staff/${safeNick}`).once("value");
            if (snap.exists()) {
                showModal("Attenzione", `Lo staff <b>${nick}</b> è già registrato nel sistema.`, null);
                return;
            }

            const avatarUrl = document.getElementById('reg-staff-avatar')?.value.trim() || "assets/logo.png";

            await db.ref(`authorized_staff/${safeNick}`).set({
                nick: nick,
                date: date,
                rank: rank,
                avatar: avatarUrl,
                addedBy: currentUser.name,
                timestamp: Date.now()
            });
            
            await db.ref(`staff/${nick}`).update({ 
                hours: 0, 
                status: "Offline", 
                rank: rank 
            });
        }
        addGlobalLog(`${currentUser.name} ha registrato lo staffer ${nick} con grado ${rank}`, 'access');
        showModal("Staff Registrato", `Lo staffer ${nick} è stato registrato con successo!`, null);
        
        document.getElementById('reg-staff-nick').value = '';
        document.getElementById('reg-staff-rank').value = '';
        if (document.getElementById('reg-staff-avatar')) document.getElementById('reg-staff-avatar').value = '';
    } catch (e) {
        console.error(e);
        showModal("Errore Database", "Impossibile salvare i dati dello staff. Verifica la connessione.", null);
    }
}

async function removeStaff() {
    const date = document.getElementById('rem-staff-date').value;
    const reason = document.getElementById('rem-staff-reason').value.trim();
    const nick = document.getElementById('rem-staff-nick').value.trim();

    if (!date || !reason || !nick) { showModal("Errore", "Compila tutti i campi per rimuovere lo staff!", null); return; }

    showModal("Conferma Rimozione", `Sei sicuro di voler eliminare @${nick} dallo staff? Non potrà più accedere.`, async () => {
        try {
            if (isCloudActive) {
                const safeNick = nick.replace(/\./g, '_');
                await db.ref(`authorized_staff/${safeNick}`).remove();
                await db.ref(`staff/${nick}`).remove();
            }
            addGlobalLog(`${currentUser.name} ha rimosso lo staffer ${nick} dal sistema [Firma: ${currentUser.name}]`, 'access');
            showModal("Eseguito", "Staff eliminato correttamente dal sistema.", null);
            document.getElementById('rem-staff-nick').value = '';
            document.getElementById('rem-staff-reason').value = '';
        } catch (e) {
            console.error(e);
            alert("Errore durante la rimozione.");
        }
    });
}

async function modifyStaffNick() {
    const reason = document.getElementById('mod-staff-reason').value.trim();
    const oldNick = document.getElementById('mod-staff-old').value.trim();
    const newNick = document.getElementById('mod-staff-new').value.trim();
    const signature = document.getElementById('mod-staff-signature').value.trim();

    if (!reason || !oldNick || !newNick || !signature) { 
        showModal("Errore", "Tutti i campi sono obbligatori!", null); 
        return; 
    }
    if (!newNick.startsWith('@') || !signature.startsWith('@')) { 
        showModal("Errore", "I nickname e la firma devono iniziare con @!", null); 
        return; 
    }

    try {
        if (isCloudActive) {
            const safeOld = oldNick.replace(/\./g, '_');
            const safeNew = newNick.replace(/\./g, '_');
            
            const snap = await db.ref(`authorized_staff/${safeOld}`).once("value");
            if (snap.exists()) {
                const data = snap.val();
                data.nick = newNick;
                
                // Salva nuovo
                await db.ref(`authorized_staff/${safeNew}`).set(data);
                // Rimuovi vecchio
                await db.ref(`authorized_staff/${safeOld}`).remove();
                
                // Trasferimento ore e sanzioni se necessario
                const hoursSnap = await db.ref(`staff/${oldNick}`).once("value");
                if (hoursSnap.exists()) {
                    await db.ref(`staff/${newNick}`).set(hoursSnap.val());
                    await db.ref(`staff/${oldNick}`).remove();
                }

                addGlobalLog(`${currentUser.name} ha cambiato il nick di ${oldNick} in ${newNick}. Motivo: ${reason} [Firma: ${signature}]`, 'access');
                showModal("Successo", "Nickname aggiornato con successo!", null);
                
                ['mod-staff-reason', 'mod-staff-old', 'mod-staff-new', 'mod-staff-signature'].forEach(id => {
                    document.getElementById(id).value = '';
                });
            } else {
                showModal("Errore", "Staffer non trovato nel sistema.", null);
            }
        }
    } catch (e) {
        console.error(e);
        showModal("Errore", "Errore durante la modifica.", null);
    }
}

async function rankUpStaff() {
    const target = document.getElementById('rank-up-target').value.trim();
    const reason = document.getElementById('rank-up-reason').value.trim();
    const role = document.getElementById('rank-up-role').value;
    const signature = document.getElementById('rank-up-signature').value.trim();

    if (!target || !reason || !role || !signature) {
        showModal("Errore", "Compila tutti i campi per il Rank Up!", null);
        return;
    }

    try {
        if (isCloudActive) {
            const safeNick = target.replace(/\./g, '_');
            const snap = await db.ref(`authorized_staff/${safeNick}`).once("value");
            
            if (snap.exists()) {
                await db.ref(`authorized_staff/${safeNick}`).update({ rank: role });
                await db.ref(`staff/${target}`).update({ rank: role });
                
                addGlobalLog(`${currentUser.name} ha promosso ${target} a ${role}. Motivo: ${reason} [Firma: ${signature}]`, 'access');
                
                showModal("✅ Rank Up Registrato", `Il Rank Up per <b>${target}</b> è stato registrato. Clicca 'Aggiorna' per confermare e ricaricare la lista.`, () => {
                    location.reload();
                }, false, true); // true per nascondere annulla, o gestirlo separatamente
                
                // Sovrascriviamo il testo dei bottoni per il Rank Up specifico
                setTimeout(() => {
                    const confirmBtn = document.getElementById('modal-confirm-btn');
                    const cancelBtn = document.getElementById('modal-cancel-btn');
                    if (confirmBtn) confirmBtn.innerText = "AGGIORNA";
                    if (cancelBtn) {
                        cancelBtn.innerText = "ANNULLA";
                        cancelBtn.style.display = "block";
                        cancelBtn.onclick = () => closeModal();
                    }
                }, 50);
            } else {
                showModal("Errore", "Staffer non trovato. Assicurati che sia registrato correttamente.", null);
            }
        }
    } catch (e) {
        console.error(e);
        showModal("Errore", "Errore durante il Rank Up.", null);
    }
}

async function resetStaffManually() {
    const target = document.getElementById('manual-reset-target').value.trim();
    if (!target) {
        showModal("Errore", "Inserisci il nome dello staffer da resettare.", null);
        return;
    }

    showModal("⚠️ CONFERMA RESET", `Sei sicuro di voler forzare lo stato OFFLINE e azzerare le ore di <b>${target}</b>?`, async () => {
        try {
            if (isCloudActive) {
                await db.ref(`staff/${target}`).update({
                    status: "Offline",
                    hours: 0,
                    startTime: null,
                    lastUpdate: new Date().toISOString()
                });
                
                addGlobalLog(`${currentUser.name} ha resettato manualmente ore e shift di ${target}`, 'access');
                showModal("✅ Reset Effettuato", `Shift disattivato e ore azzerate per ${target}.`, () => {
                    location.reload();
                });
            }
        } catch (e) {
            console.error(e);
            showModal("Errore", "Impossibile comunicare con il database.", null);
        }
    });
}

// --- STAFF LIST PAGE ---
const RANK_HIERARCHY = [
    "Founder", "Assistant Founder", "Fondazione", "Co-Founder",
    "Amministrazione", "Owner", "Co-Owner", "Gradi Gestionali", "Staff Manager", "Staff Supervisor",
    "Gradi Alti", "Head Admin", "Senior Admin", "Admin", "Trial Admin",
    "Gradi Bassi", "Head Moderator", "Senior Moderator", "Moderator", "Trial Moderator", "Trial Staff"
];

async function updateStaffListPage() {
    const body = document.getElementById('staff-list-full-body');
    if (!body) return;

    try {
        let authorized = {};
        let realTimeStatus = {};

        if (isCloudActive) {
            const authSnap = await db.ref("authorized_staff").once("value");
            if (authSnap.exists()) authorized = authSnap.val();
            
            const statusSnap = await db.ref("staff").once("value");
            if (statusSnap.exists()) realTimeStatus = statusSnap.val();
        }

        // ORDINAMENTO PER GRADO E POI ALFABETICO
        const staffArray = Object.values(authorized).sort((a, b) => {
            const rankA = RANK_HIERARCHY.indexOf(a.rank);
            const rankB = RANK_HIERARCHY.indexOf(b.rank);
            if (rankA !== rankB) return rankA - rankB;
            return a.nick.localeCompare(b.nick);
        });

        renderStaffTable(staffArray, realTimeStatus);
    } catch (e) {
        console.error(e);
        body.innerHTML = '<tr><td colspan="4" style="text-align:center; color: var(--accent-red);">Errore nel caricamento della lista.</td></tr>';
    }
}

function renderStaffTable(staffArray, statusMap) {
    const body = document.getElementById('staff-list-full-body');
    if (staffArray.length === 0) {
        body.innerHTML = '<tr><td colspan="4" style="text-align:center; opacity:0.5;">Nessuno staff registrato.</td></tr>';
        return;
    }

    body.innerHTML = staffArray.map(s => {
        // Controllo Stato Attivo: Se è l'utente corrente o se lo stato in DB è 'In Servizio'
        let status = statusMap[s.nick]?.status || "Offline";
        const isActive = (s.nick === currentUser.name) || (status === "In Servizio");
        
        const statusClass = isActive 
            ? 'background: rgba(46,204,113,0.15); color: #2ecc71; border: 1px solid rgba(46,204,113,0.4); box-shadow: 0 0 10px rgba(46,204,113,0.1);' 
            : 'background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.4); border: 1px solid rgba(255,255,255,0.08);';
        
        const label = isActive ? "ATTIVO" : "OFFLINE";
        const avatar = s.avatar || 'assets/logo.png';
        
        return `
            <tr style="border-bottom: 1px solid rgba(255,255,255,0.03); vertical-align: middle;">
                <td style="padding: 15px 10px; display: flex; align-items: center; gap: 15px;">
                    <img src="${avatar}" style="width: 40px; height: 40px; border-radius: 10px; border: 2px solid var(--accent-purple); object-fit: cover; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
                    <div style="display: flex; flex-direction: column; justify-content: center;">
                        <span style="font-weight: 700; font-size: 0.95rem; color: #fff;">${s.nick}</span>
                        <small style="opacity: 0.4; font-size: 0.7rem; letter-spacing: 0.5px;">ID: ${s.discordId || '---'}</small>
                    </div>
                </td>
                <td style="padding: 15px 10px; font-size: 0.85rem; opacity: 0.7; font-weight: 500;">${s.date}</td>
                <td style="padding: 15px 10px;">
                    <span class="status-tag" style="background: rgba(123,44,191,0.1); color: var(--accent-purple); border: 1px solid rgba(123,44,191,0.3); font-weight: 700; font-size: 0.7rem; letter-spacing: 0.5px; padding: 4px 12px; border-radius: 6px;">${(s.rank || 'N/D').toUpperCase()}</span>
                </td>
                <td style="padding: 15px 10px;">
                    <span class="status-tag" style="${statusClass} font-weight: 800; font-size: 0.7rem; padding: 4px 12px; border-radius: 6px;">${label}</span>
                </td>
            </tr>
        `;
    }).join('');
}

function filterStaffList() {
    const query = document.getElementById('staff-list-search').value.toLowerCase();
    const rows = document.querySelectorAll('#staff-list-full-body tr');
    
    rows.forEach(row => {
        const text = row.innerText.toLowerCase();
        row.style.display = text.includes(query) ? '' : 'none';
    });
}

// --- LOGS ---
async function renderGlobalLogs(cloudLogs = null) {
    let logs = [];
    if (cloudLogs) {
        logs = cloudLogs;
    } else if (isCloudActive) {
        const snapshot = await db.ref("logs").limitToLast(50).once("value");
        if (snapshot.exists()) {
            snapshot.forEach(child => { logs.unshift(child.val()); });
        }
    } else {
        logs = JSON.parse(localStorage.getItem('wash_rp_global_logs') || '[]');
    }

    const cS = document.getElementById('log-staff-service');
    const cA = document.getElementById('log-access');
    const cSn = document.getElementById('log-sanctions');
    if (!cS || !cA || !cSn) return;
    
    cS.innerHTML = ''; cA.innerHTML = ''; cSn.innerHTML = '';
    logs.forEach(log => {
        const timeStr = new Date(log.time).toLocaleTimeString();
        const html = `<div style="font-size: 0.7rem; padding: 6px; border-bottom: 1px solid rgba(255,255,255,0.03);">
                        <span style="color: var(--accent-purple); font-weight: 600;">[${timeStr}]</span> ${log.msg}
                      </div>`;
        
        if (log.type === 'staff-service') cS.innerHTML += html;
        else if (log.type === 'access') cA.innerHTML += html;
        else if (log.type === 'sanctions') cSn.innerHTML += html;
    });
}

async function addGlobalLog(msg, type) {
    const log = { msg, type, time: new Date().toISOString(), staff: currentUser ? currentUser.name : "Sistema" };
    
    // Log Locale/Cloud
    if (isCloudActive) {
        await db.ref("logs").push(log);
    } else {
        let logs = JSON.parse(localStorage.getItem('wash_rp_global_logs') || '[]');
        logs.unshift(log);
        if (logs.length > 200) logs.pop();
        localStorage.setItem('wash_rp_global_logs', JSON.stringify(logs));
        if (document.getElementById('admin-content').style.display === 'block') renderGlobalLogs();
    }

    // --- DISCORD WEBHOOK LOGGING ---
    sendToDiscord(msg, type);
}

async function sendToDiscord(message, type) {
    if (!CONFIG.WEBHOOK_URL) return;

    let color = 0x7b2cb7; // Viola Default
    if (type === 'sanctions') color = 0xe74c3c; // Rosso
    if (type === 'access') color = 0x3498db; // Blu
    if (type === 'staff-service') color = 0x2ecc71; // Verde

    const embed = {
        title: `LOG DASHBOARD: ${type.toUpperCase()}`,
        description: message,
        color: color,
        timestamp: new Date().toISOString(),
        footer: { text: "Washington RP System" }
    };

    try {
        await fetch(CONFIG.WEBHOOK_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ embeds: [embed] })
        });
    } catch (e) {
        console.error("Errore invio Webhook:", e);
    }
}

async function clearAllInactivity() {
    showModal("⚠️ ATTENZIONE", "Stai per eliminare TUTTE le richieste di inattività (pendenti e attive). Sei sicuro?", async () => {
        try {
            if (isCloudActive) {
                await db.ref("inactivity").remove();
                addGlobalLog(`${currentUser.name} ha azzerato l'intero database delle inattività.`, 'access');
                showModal("Eseguito", "Tutte le inattività sono state rimosse.", () => {
                    location.reload();
                });
            }
        } catch (e) {
            console.error(e);
            showModal("Errore", "Impossibile resettare le inattività.", null);
        }
    });
}

// --- OTHERS ---
async function resetAllData() {
    showModal("⚠️ RESET TOTALE", "Stai per eliminare TUTTI i dati (Staff, Sanzioni, Log, Inattività). Questa azione è IRREVERSIBILE. Vuoi procedere?", async () => {
        if (!isCloudActive) return;

        try {
            // Elimina tutto tranne la configurazione
            await db.ref("authorized_staff").remove();
            await db.ref("staff").remove();
            await db.ref("logs").remove();
            await db.ref("subjects").remove();
            await db.ref("notifications").remove();
            await db.ref("inactivity").remove();
            await db.ref("strikes").remove();

            // Reinserisci Admin Iniziale
            const safeAdmin = DEFAULT_ADMIN.replace(/\./g, '_');
            await db.ref(`authorized_staff/${safeAdmin}`).set({
                nick: DEFAULT_ADMIN,
                rank: "Founder",
                avatar: "assets/logo.png",
                date: new Date().toLocaleDateString()
            });

            showModal("✅ Reset Completato", "Il database è stato azzerato. Ricarica la pagina.", () => {
                location.reload();
            });
        } catch (e) {
            console.error(e);
            showModal("Errore", "Errore durante il reset dei dati.", null);
        }
    });
}

function updateFormStyle() {
    const type = document.getElementById('sanc-type').value;
    const card = document.querySelector('#sanzioni .card');
    const dateField = document.getElementById('ban-dates');
    const btn = document.querySelector('#sanzioni .login-btn');
    
    if (!card) return;
    card.className = 'card'; 
    
    if (type === 'ban') { 
        card.classList.add('sanction-ban'); 
        if (dateField) dateField.style.display = 'block'; 
        if (btn) btn.innerText = "INVIA BAN";
    }
    else if (type === 'warn') {
        card.classList.add('sanction-warn'); 
        if (dateField) dateField.style.display = 'none'; 
        if (btn) btn.innerText = "INVIA WARN";
    }
    else { 
        card.classList.add('sanction-warn'); 
        if (dateField) dateField.style.display = 'none'; 
        if (btn) btn.innerText = "INVIA COMUNICAZIONE";
    }
}

async function syncData() {
    // Gestito dai listener .on() di Firebase
}

// --- ROBLOX INTEGRATION ---
function debounceRobloxPreview(username, imgId, suggestionsId) {
    clearTimeout(robloxTimeout);
    
    const img = document.getElementById(imgId);
    const sugg = document.getElementById(suggestionsId);

    // ABILITATO DA 2 CARATTERI
    if (!username || username.length < 2 || username.startsWith('@')) {
        if (img) img.style.display = 'none';
        if (sugg) { sugg.innerHTML = ''; sugg.style.display = 'none'; }
        return;
    }
    robloxTimeout = setTimeout(() => {
        updateRobloxPreview(username, imgId);
        showRobloxSuggestions(username, suggestionsId, imgId);
    }, 500);
}

async function showRobloxSuggestions(query, suggestionsId, imgId) {
    const sugg = document.getElementById(suggestionsId);
    if (!sugg) return;

    try {
        const res = await fetch(`https://users.roproxy.com/v1/users/search?keyword=${query}&limit=5`);
        if (!res.ok) throw new Error("Roblox Search API failed");
        const data = await res.json();

        if (data.data && data.data.length > 0) {
            const userIds = data.data.map(u => u.id);
            const thumbRes = await fetch(`https://thumbnails.roproxy.com/v1/users/avatar-headshot?userIds=${userIds.join(',')}&size=150x150&format=Png&isCircular=true`);
            const thumbData = await thumbRes.json();
            
            sugg.innerHTML = data.data.map((user, idx) => {
                const thumbItem = thumbData.data ? thumbData.data.find(t => t.targetId === user.id) : null;
                const thumb = thumbItem ? thumbItem.imageUrl : 'assets/logo.png';
                return `
                    <div class="suggestion-item" onclick="selectRobloxUser('${user.name}', '${thumb}', '${suggestionsId}', '${imgId}')">
                        <img src="${thumb}" alt="Avatar">
                        <div>
                            <div class="name">${user.name}</div>
                            <div class="display-name">${user.displayName}</div>
                        </div>
                    </div>
                `;
            }).join('');
            sugg.style.display = 'block';
            sugg.style.opacity = '1';
        } else {
            sugg.style.display = 'none';
        }
    } catch (e) {
        console.error("Errore suggerimenti Roblox:", e);
        // Fallback: prova senza miniature se il caricamento fallisce
        if (e.message !== "Roblox Search API failed") {
            // Se abbiamo i dati ma non le miniature, mostriamoli comunque
            // Ma per ora nascondiamo per evitare UI rotta
            sugg.style.display = 'none';
        }
    }
}

function selectRobloxUser(username, thumb, suggestionsId, imgId) {
    let inputId = 'search-input';
    if (suggestionsId.includes('sanc')) inputId = 'sanc-subject';
    if (suggestionsId.includes('admin')) inputId = 'admin-search-input';
    
    const input = document.getElementById(inputId);
    if (input) input.value = username;
    
    document.getElementById(suggestionsId).style.display = 'none';
    const img = document.getElementById(imgId);
    if (img) {
        img.src = thumb;
        img.style.display = 'block';
    }
    // Salviamo l'avatar selezionato globalmente per l'invio della sanzione
    selectedRobloxAvatar = thumb;
}

async function updateRobloxPreview(username, imgId) {
    const img = document.getElementById(imgId);
    if (!img) return;

    try {
        const userRes = await fetch(`https://users.roproxy.com/v1/usernames/users`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ usernames: [username], excludeBannedUsers: false })
        });
        const userData = await userRes.json();

        if (userData.data && userData.data.length > 0) {
            const userId = userData.data[0].id;
            const thumbRes = await fetch(`https://thumbnails.roproxy.com/v1/users/avatar-headshot?userIds=${userId}&size=150x150&format=Png&isCircular=true`);
            const thumbData = await thumbRes.json();

            if (thumbData.data && thumbData.data.length > 0) {
                img.src = thumbData.data[0].imageUrl;
                img.style.display = 'block';
                return;
            }
        }
        img.style.display = 'none';
    } catch (e) {
        console.error("Errore recupero avatar Roblox:", e);
        img.style.display = 'none';
    }
}
// --- AUTO-CHECK EXPIRATIONS ---
async function autoCheckExpiredInactivity() {
    if (!isCloudActive) return;
    
    try {
        const now = new Date();
        const inaSnap = await db.ref("inactivity").once("value");
        if (inaSnap.exists()) {
            inaSnap.forEach(child => {
                const r = child.val();
                if (r.status === 'approved' && new Date(r.end) < now) {
                    // Inattività scaduta
                    addNotification(r.user, `Il tuo periodo di inattività è TERMINATO. Bentornato in servizio!`);
                    db.ref(`inactivity/${child.key}`).update({ status: 'expired' });
                }
            });
        }

        const subSnap = await db.ref("subjects").once("value");
        if (subSnap.exists()) {
            subSnap.forEach(subjectChild => {
                const logs = Object.entries(subjectChild.val());
                logs.forEach(async ([id, log]) => {
                    if (log.type === 'ban' && log.expiry && new Date(log.expiry) < now && !log.expiredNotified) {
                        addNotification(log.subject, `Il tuo BAN è scaduto. Ora puoi rientrare nel server.`);
                        db.ref(`subjects/${subjectChild.key}/${id}`).update({ expiredNotified: true });
                    }
                });
            });
        }
    } catch (e) {
        console.error("Errore auto-check:", e);
    }
}
